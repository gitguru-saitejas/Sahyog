...
import { FamilySelector } from "../components/auth/FamilySelector";
import { Activity, LogOut, Sun, Moon } from "lucide-react";

export const FamilySelection = () => {
  const { selectPatient, logout, theme, toggleTheme } = useAuth();
    ...
    <div className="min-h-screen flex flex-col justify-between bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-50 transition duration-300 relative overflow-hidden">
      ...
      <header className="px-6 py-4 flex items-center justify-between border-b border-slate-100 dark:border-slate-850 shrink-0 z-10 bg-white/40 dark:bg-slate-950/40 backdrop-blur-md">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <button
            onClick={toggleTheme}
            className="p-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800/80 transition active:scale-95 duration-250 cursor-pointer shadow-sm text-slate-600 dark:text-slate-350"
          >
          ...
          <button
            ...
            className="flex items-center gap-2 py-2.5 px-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-red-600 dark:text-red-400 font-semibold text-sm rounded-xl hover:bg-red-50 dark:hover:bg-red-950/20 hover:border-red-200 transition active:scale-95 duration-200 cursor-pointer shadow-sm"
          >
      ...
      <main className="flex-1 flex items-center justify-center py-10 z-10">
        <div className="w-full animate-in fade-in zoom-in-95 duration-500">
          <FamilySelector onSelect={handleSelect} />
        </div>
      </main>
