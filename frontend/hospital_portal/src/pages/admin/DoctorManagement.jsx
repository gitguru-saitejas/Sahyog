...
import {
  Stethoscope, Plus, Trash2, Key, X, Eye, Search, RefreshCcw
} from "lucide-react";
import { Card } from "../../components/ui/Card";
import { Badge } from "../../components/ui/Badge";
import { Avatar } from "../../components/ui/Avatar";

// ─── Reusable styled input for modal forms ───────────────────────────────────
...
export default function DoctorManagement() {
  ...
  return (
    <div className="space-y-6 font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-[#64748B] dark:text-slate-400 mt-0.5">
            Configure medical staff directories, licenses, and credentials
          </p>
        </div>
                ...
                <tr>
                  <td colSpan={6}>
                    <div className="flex flex-col items-center justify-center py-14 gap-3 text-center">
                      ...
                      <p className="font-bold text-sm text-[#1E293B] dark:text-slate-200">No doctors found</p>
                      <p className="text-xs text-[#64748B]">
                        {searchQuery ? "Try adjusting your search query." : "Add a doctor to get started."}
                      </p>
                    </div>
                  </td>
                </tr>
                  ...
                  <tr key={idx} className="hover:bg-[#F8FAFC] dark:hover:bg-slate-850 transition">
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-bold text-[#1E293B] dark:text-slate-100">{doc.first_name} {doc.last_name}</p>
                          <p className="text-[10px] font-mono text-[#94A3B8] mt-0.5">{doc.employee_id}</p>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-5 py-3.5">
                      ...
                      >
                        <Badge color={doc.status === "ACTIVE" ? "green" : "red"} dot>
                          {doc.status}
                        </Badge>
                      </button>
                    </td>
      ...
      {/* ── Add Doctor Modal ─────────────────────────────────────────────────── */}
      {isAddModalOpen && (
        <Modal title="Add New Doctor Profile" onClose={() => setIsAddModalOpen(false)}>
          <form onSubmit={addForm.handleSubmit(onAddSubmit)} className="grid grid-cols-2 gap-4">
            ...
            <FormInput label="Specialization">
              <input required type="text" placeholder="e.g. Cardiology" className={inputCls} {...addForm.register("specialization")} />
            </FormInput>
            <FormInput label="Qualification">
              <input required type="text" placeholder="e.g. MD, MBBS" className={inputCls} {...addForm.register("qualification")} />
            </FormInput>
            <FormInput label="License Number">
              <input required type="text" className={inputCls} {...addForm.register("license_number")} />
            </FormInput>
            <div className="col-span-2 flex justify-end gap-2 pt-2 border-t border-[#E5E7EB] dark:border-slate-800">
              <button type="button" onClick={() => setIsAddModalOpen(false)}
                className="py-2 px-4 bg-[#F1F5F9] dark:bg-slate-800 text-[#64748B] dark:text-slate-300 font-bold text-xs rounded-xl hover:bg-[#E2E8F0] transition cursor-pointer">
                Cancel
              </button>
              <button type="submit"
      ...
      {isPasswordModalOpen && (
        <Modal title="Reset Password" onClose={() => setIsPasswordModalOpen(false)}>
          <p className="text-xs text-[#64748B] font-semibold">
            Resetting credentials for doctor <span className="font-mono font-bold text-[#1E293B] dark:text-slate-200">{selectedUser?.employee_id}</span>.
          </p>
      ...
      {isViewModalOpen && selectedUser && (
        <Modal title="Doctor Profile" onClose={() => setIsViewModalOpen(false)}>
          ...
          <div className="space-y-3 text-xs">
            {[
              ...
              { label: "Qualification",  value: selectedUser.qualification },
              { label: "License Number", value: selectedUser.license_number, mono: true },
      ...
      {createdCredentials && (
        <Modal title="Doctor Created Successfully" onClose={() => setCreatedCredentials(null)}>
